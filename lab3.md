# Tutorial 6 — Mixed-Precision Search (Tasks 1–2)
...
In the original Tutorial 6 setup, all layers mapped to `LinearInteger` share the *same* width and fractional-width. This is suboptimal because different `Linear` layers can have different quantization sensitivity.
...
- In this run, the best-performing group clusters near the top curve band, while some precision types lag (notably the pure binary variant in this configuration).
